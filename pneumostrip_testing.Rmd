...
```{r}
a1 <- read_excel('./Data/ps_data.xlsx')

a1.m <- reshape2::melt(a1, id.vars=c('Pool'))
...
p1 <- ggplot(a1.m[!(a1.m$variable %in% c('ply','CpsA','auto')),], aes(Pool, variable, fill= value)) +
  geom_tile() +
  theme_classic()+
...
p2 <- ggplot(b1, aes(Pool, variable, fill= value)) +
  geom_tile() +
  theme_classic()+
  ylab('Gene target')+
